...
trait Terminal: Debug + Hash + Eq + Clone + Display {}
...
enum GrammarToken<T: Terminal, NT: NonTerminal> {
    Terminal(T),
    NonTerminal(NT),
}
...
#[derive(Debug, Hash, PartialEq, Eq, Clone)]
enum TokenOrEof<T: Terminal, NT: NonTerminal> {
    Terminal(T),
    NonTerminal(NT),
    EOF,
}
...
struct GrammarRule<T: Terminal, NT: NonTerminal> {
    lhs: NT,
    rhs: Vec<GrammarToken<T, NT>>,
}

const RULE_ALIGNMENT: usize = 14;

struct Grammar<T: Terminal, NT: NonTerminal> {
    rules: Vec<GrammarRule<T, NT>>,
    rules_by_lhs: HashMap<NT, Vec<usize>>,
}

impl<T: Terminal, NT: NonTerminal> Grammar<T, NT> {
    fn new(rules: Vec<GrammarRule<T, NT>>) -> Self {
...
enum Action<'a, T: Terminal, NT: NonTerminal> {
    Shift(usize),
...
struct ParsingTable<'a, T: Terminal, NT: NonTerminal> {
    table: HashMap<(usize, TokenOrEof<T, NT>), Action<'a, T, NT>>,
}

impl<'a, T: Terminal, NT: NonTerminal> ParsingTable<'a, T, NT> {
    fn dump(&self, grammar: &'a Grammar<T, NT>) {
...
fn generate_parsing_table<'a, T: Terminal, NT: NonTerminal>(
    grammar: &'a Grammar<T, NT>
) -> ParsingTable<'a, T, NT> {
    grammar.dump();
...
trait InputToken<T: Terminal> {
    fn kind(&self) -> T;
}

enum InputTokenOrAst<T: Terminal, I: InputToken<T>, A> {
    InputToken(I, PhantomData<T>),
    Ast(A),
}

#[derive(Debug)]
struct ParsingError<T: Terminal, NT: NonTerminal> {
    item_set: usize,
    token: TokenOrEof<T, NT>,
}

fn parse<T: Terminal, NT: NonTerminal, I: InputToken<T>, A>(
